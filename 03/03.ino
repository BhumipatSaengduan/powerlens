...
// ============================================================================
//  PowerLens SM-PE-03 — ESP32 side only
//  RAW stable-rate BINARY CHUNK MODE + SD raw log
//
//  Flow:
//  - ESP32 connects WiFi only for NTP and the WiFi status LED
//  - Sync NTP for daily SD filenames
//  - Request raw electrical data from Pi/EF via UART at an ADC-safe cadence
...
// This gives the async SD writer about 2.8 seconds of stall tolerance at ~700 Hz
...
// Pi waits for a fresh ADC snapshot before answering. Keep this longer than
// one sample period so stable data is preferred over silent UART timeouts.
static const uint32_t EF_RESPONSE_TIMEOUT_US = 12000UL;
...
//
// Schedule:
//   Production mode: keep Pi/ADC acquisition at an ADC-safe stable cadence.
...
//
// Architecture note: piSerial is exclusive to adcTask (raw request path).
...
static const RateScheduleEntry RATE_SCHEDULE[] = {
    { 0, SAMPLE_PERIOD_US, "Day 1+: ADC-safe raw cadence (~700 Hz)" },
    { 1, SAMPLE_PERIOD_US, "Day 1+: ADC-safe raw cadence (~700 Hz)" },
    { 2, SAMPLE_PERIOD_US, "Day 1+: ADC-safe raw cadence (~700 Hz)" },
};
...
// avoids calling the WiFi driver from the time-critical UART request path.
...
void adcTask(void* parameter) {
    ...
    for (;;) {
        ...
        // them onto the same raw UART transaction. This keeps raw timing stable while
...
#if ENABLE_PI_METRICS_PACKET
    Serial.println("[BOOT] Mode: RAW stable-rate binary 250-sample chunk + SD raw log + AWS feature packet");
    Serial.println("[BOOT] ESP32 requests Pi/EF raw+metrics over UART, stores raw locally, and publishes features to AWS IoT");
#else
    Serial.println("[BOOT] Mode: RAW-only stable-rate binary 250-sample chunk + SD raw log; Pi feature compute disabled");
