...
unsigned int status_wait_counter = 0;
const unsigned int status_wait_max_count = 3;
...
bool MCP3x6x::begin(uint16_t channelmask, float vref) {
  ...
  _status = reset();
  setClockSelection(clk_sel::INTERN_OUTPUT);          // todo make configurable by function parameter
  ...
  // set ADC mode to standby -- ready for conversion
  _status = standby(); //_fastcmd(MCP3x6x_CMD_STANDBY);
...
#ifdef MCP3x6x_DEBUG
  Serial.println("set clock INTERNAL");
#endif
  _status              = write(settings.config0);
#ifdef MCP3x6x_DEBUG
  Serial.println("exit setClockSelection(clk)");
#endif
}

void MCP3x6x::enableScanChannel(mux_t ch) {
    uint8_t channel = (uint8_t)ch.vin_plus;

    if(channel < 8)
    {
        settings.scan.channel.single_ended |= (1 << channel);
    }
...
void MCP3x6x::ks_mode_setup() {

    setAveraging(osr::OSR_32);
...
void MCP3x6x::ks_mode_standby() {
    _status = standby();
}

void MCP3x6x::convertMux(mux_t ch) {
    settings.mux = ch;
    _status = write(settings.mux);
    setConversionMode(conv_mode::ONESHOT_STANDBY);
    _status = conversion(); // will return to standby for one-shot mode
}

int32_t MCP3x6x::analogReadMux(mux_t ch) {
        _status = read(&adcdata);
        return result.raw[(uint8_t)adcdata.channelid] = adcdata.value;
}

int32_t MCP3x6x::analogReadScan(mux_t ch) {
    Adcdata data;

    _status = read(&data);

    result.raw[data.channelid] = data.value;

    return result.raw[(uint8_t)ch.vin_plus];
}
...
int32_t MCP3x6x::analogRead(mux_t ch) {
  ...
  if (settings.scan.channel.raw == 0) {
    ...
    settings.mux = ch;
#ifdef MCP3x6x_DEBUG
    Serial.println("write settings.mux");
#endif
    _status      = write(settings.mux);
#ifdef MCP3x6x_DEBUG
    Serial.println("conversion : _fastcmd(MCP3x6x_CMD_CONVERSION)");
#endif
    _status      = conversion();

    
#ifdef MCP3x6x_DEBUG
    Serial.println("readloop");
#endif
    status_wait_counter = 0;
    while (!_status.dr) { // IRQ does not seem to be implemented??? ks
#ifdef MCP3x6x_DEBUG
        Serial.println("read(&adcdata)");
#endif
      _status = read(&adcdata);

      if (status_wait_counter++ >= status_wait_max_count) {
#ifdef MCP3x6x_DEBUG
...
#ifdef MCP3x6x_DEBUG
    Serial.println("returning from analogRead");
#endif
    return result.raw[(uint8_t)adcdata.channelid] = adcdata.value;
  }
  ...
  result.raw[adcdata.channelid] = adcdata.value;

  return result.raw[adcdata.channelid];
}
